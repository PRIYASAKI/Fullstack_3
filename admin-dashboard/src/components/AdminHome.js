...
const AdminHome = () => {
  ...
  const markNotificationAsRead = async (id) => {
    try {
      await axios.put(`${API_URL}/api/admin/notifications/${id}/read`)
  ...
  const markAllNotificationsAsRead = async () => {
    try {
      await axios.put(`${API_URL}/api/admin/notifications/read-all`)
  ...
  const markMessageAsRead = async (id) => {
    try {
      await axios.put(`${API_URL}/api/admin/messages/${id}/read`)
  ...
  const handleReplySubmit = async () => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/api/admin/messages/${selectedMessage._id}/reply`, {
  ...
  useEffect(() => {
    const ws = new WebSocket(API_URL.replace(/^http/, "ws")); // Use ws(s) protocol for WebSocket
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const bookingsResponse = await axios.get(`${API_URL}/api/admin/bookings`)
